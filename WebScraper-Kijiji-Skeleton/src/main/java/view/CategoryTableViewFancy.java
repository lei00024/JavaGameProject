 ...
 * @author leizhe
 */
@WebServlet(name = "CategoryTableFancy", urlPatterns = {"/CategoryTableFancy"})
public class CategoryTableViewFancy extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(searchText!=null&&request.getParameter("search")!=null){
            List<Category> cat_list=new CategoryLogic().search(searchText);
            if(cat_list.size()>0){
                out.println("Search Result(s):");
                out.println("<br>");
                out.println("<br>");
                for(Category cat:cat_list){
                    out.println("Id: "+cat.getId()+"<br>"+"Url: "+cat.getUrl()+"<br>"+"Title: "+cat.getTitle());
                    out.println("<br>");
                    out.println("<br>");
                }
            out.println("Total Category(s) found: "+cat_list.size());
            }else{
        ...
        if(delArray.length!=0){
            for(int i=0;i<delArray.length;i++){
                int catId=Integer.parseInt(delArray[i]);
                Category cat=new CategoryLogic().getWithId(catId);
                new CategoryLogic().delete(cat);
                
                Category cat_test=new CategoryLogic().getWithId(catId);
                if(cat_test==null){
                    out.println("Selected Category "+i+" deleted!");
                    out.println("<br>");
                }else{
    ...
    public String getServletInfo() {
        return "Sample of Category View Fancy";
    }
