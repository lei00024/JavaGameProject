 ...
 * @author leizhe
 */
@WebServlet(name = "ImageTableFancy", urlPatterns = {"/ImageTableFancy"})
public class ImageTableViewFancy extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>CategoryViewFancy</title>");
        ...
        if(searchText!=null&&request.getParameter("search")!=null){
            List<Image> img_list=new ImageLogic().search(searchText);
            if(img_list.size()>0){
                out.println("Search Result(s):");
                out.println("<br>");
                out.println("<br>");
                for(Image img:img_list){
                    out.println("Id: "+img.getId()+"<br>"+"Url: "+img.getUrl()+"<br>"+""
                            + "Path: "+img.getPath() + "<br>"+"Name: "+img.getName());
                    out.println("<br>");
                    out.println("<br>");
                }
            out.println("Total Category(s) found: "+img_list.size());
            }else{
        ...
        if(delArray.length!=0){
            for(int i=0;i<delArray.length;i++){
                int imgId=Integer.parseInt(delArray[i]);
                Image img=new ImageLogic().getWithId(imgId);
                new ImageLogic().delete(img);
                
                Image img_test=new ImageLogic().getWithId(imgId);
                if(img_test==null){
                    out.println("Selected Image "+i+" deleted!");
                    out.println("<br>");
                }else{
        ...
        //bonus (update); single update only
       // String edit=request.getParameter("edit");
//        if(edit!=null&&boxClicked!=null){
//            
//        }
    ...
    public String getServletInfo() {
        return "Sample of Image View Fancy";
    }
