...
public class AccountTableViewFancy extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Account> acc_list=new AccountLogic().search(searchText);
        if(acc_list.size()>0){
            out.println("Search Result(s):");
            out.println("<br>");
            out.println("<br>");
            for(Account act:acc_list){
                out.println("Id: "+act.getId()+"<br>"+"DisplayName: "+act.getDisplayName()+"<br>"+"User: "+act.getUser());
                out.println("<br>");
                out.println("<br>");
            }
            out.println("Total account(s) found: "+acc_list.size());
        }else{
        ...
        if(delArray.length!=0){
            for(int i=0;i<delArray.length;i++){
                int accId=Integer.parseInt(delArray[i]);
                Account acc=new AccountLogic().getWithId(accId);
                new AccountLogic().delete(acc);
                
                Account acc_test=new AccountLogic().getWithId(accId);
                if(acc_test==null){
                    out.println("Selected Account "+i+" deleted!");
                    out.println("<br>");
                }else{
