...
public class AccountLogic extends GenericLogic<Account,AccountDAL>{
    ...
    public Account createEntity(Map<String, String[]> parameterMap) {
        ...
        if (parameterMap.containsKey(DISPLAY_NAME) && parameterMap.get(DISPLAY_NAME) != null){
            ...
            if (displayName.isEmpty() || displayName.length() > 45){
                throw new ValidationException("Invalid displayName input, must between 1 and 45 characters");
            }
            account.setDisplayName(parameterMap.get(DISPLAY_NAME)[0]);
        } else {
            throw new ValidationException("DisplayName not exist");
        }
        //setUser
        if (parameterMap.containsKey(USER) && parameterMap.get(USER) != null){
            String user = parameterMap.get(USER)[0];
            if (user.isEmpty() || user.length() > 45){
                throw new ValidationException("Invalid user input, must between 1 and 45 characters");
            }
        ...
        if (parameterMap.containsKey(PASSWORD) && parameterMap.get(PASSWORD) != null){
            String pwd = parameterMap.get(PASSWORD)[0];
            if (pwd.isEmpty() || pwd.length() > 45){
                throw new ValidationException("Invalid Password input, must between 1 and 45 characters");
            }
            account.setPassword(parameterMap.get(PASSWORD)[0]);
        } else {
            throw new ValidationException("Password not exist");
        }        
    ...
    public List<String> getColumnNames() {
        return Arrays.asList("id", "displayName", "user", "password");
    }
