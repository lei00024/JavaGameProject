...
@NamedQueries({
    ...
    @NamedQuery(name = "Account.findContaining", query = "SELECT a FROM Account a WHERE a.displayName like CONCAT('%', :search, '%') or a.user like CONCAT('%', :search, '%') or"
            + " a.id like CONCAT('%', :search, '%') or a.password like CONCAT('%', :search, '%')")})
